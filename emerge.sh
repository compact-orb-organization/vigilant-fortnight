...
mkdir /mnt/binpkgs
rclone --config /etc/portage/rclone.conf --s3-access-key-id $S3_ACCESS_KEY_ID --s3-endpoint $S3_ENDPOINT --s3-secret-access-key $S3_SECRET_ACCESS_KEY mount 1:$S3_BUCKET /mnt/binpkgs --allow-other --daemon --no-checksum --no-modtime --no-seek --read-only --vfs-cache-mode full
